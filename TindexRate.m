...
for vv = 1:Vlen
    for nn = 1:(Tindex-Startindex)
        ...
        nn
        for mm = 1:MonteCalo
            channelH = squeeze(channel(mm,nn,:));
            ...
            if methods(1) == 1
                ...
                channelHLsBf = sqrt(para.N)*channelHLs/ norm(channelHLs);
                LS_gain = abs(channelH'*channelHLsBf)^2;
                Rate(nn,1) = Rate(nn,1) + log2(1+LS_gain/SigmaN);
            end
            if methods(2) == 1
                delta_angle = 128;
                ...
                Rate(nn,2) = Rate(nn,2) + log2(1+far_field_rate_gain/SigmaN);

            end

            if methods(3) == 1
                BfvCSI = sqrt(para.N)*channelH/norm(channelH);

                PerfCSIgain = norm(channelH*BfvCSI','fro')^2;
                % Rate(ii,3) = Rate(ii,3) + log2(1+PerfCSIgain/SigmaN);
                Rate(nn,3) = Rate(nn,3) + log2(1+PerfCSIgain/SigmaN);
            end
            if methods(4) == 1
                %% LLM based beamforming
                BfvLLM = squeeze(Bfv(mm, nn,:));

                PerfCSIgainLLM = norm(channelH'*BfvLLM,'fro')^2;
                snr = 10^(snrdB/10);
